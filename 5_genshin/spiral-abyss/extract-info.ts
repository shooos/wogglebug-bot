(() => {
  ...
  Genshin.spiralAbyss!.extractInfo = (releasePost): Genshin.SpiralAbyssInfo[] => {
    Utils.info(`Start extracting Spiral Abyss information | PostId=${releasePost.id}`);
    ...
    if (updateText == null) {
      Utils.warn('Not found to Spyral Abyss Information');
      return [];
    }
    ...
    rawTexts.forEach(raw => {
      ...
      if (blessingOfTheAbyssMoon == null) {
        Utils.warn('Blessing of the Abyss moon is not found.');
        return [];
      }
    ...
    Utils.info(`Finish extracting Spiral Abyss information | Results=${JSON.stringify(results)}`);
