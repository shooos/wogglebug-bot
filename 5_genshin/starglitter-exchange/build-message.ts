(() => {
  ...
  Genshin.starglitterExchange!.buildMessage = (currentDate) => {
    Utils.info(`Start build starglitter exchange message | CurrentDate=${Utils.formatDateToIsoString(currentDate)}`);
...
スターライトのご利用は計画的に！ #原神`;
    ...
    Utils.info(`Completed build starglitter exchange message | Body=${body} `);
