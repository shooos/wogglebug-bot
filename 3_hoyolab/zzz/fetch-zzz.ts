(() => {
  HoYoLAB.ZZZ.fetchNewArrivals = (lastPostedId) => {
    ...
    Utils.info(`Finish fetching ZenlessZoneZero new arrival official posts | PostIds=${JSON.stringify(newArrivals.map(c => c.id))}`);

    return newArrivals;
  }

  HoYoLAB.ZZZ.fetchTodaysPosts = (currentDate) => {
    Utils.info(`Start fetching ZZZ today's official posts | CurrentDate=${Utils.formatDateToIsoString(currentDate)}`);
    ...
    Utils.info(`Finish fetching ZZZ today's official posts | Count=${results.length}`);
